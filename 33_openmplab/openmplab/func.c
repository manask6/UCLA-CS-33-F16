			...
			int Ones, int iter, int X, int Y, int Z, int n)
{
	...
	#pragma omp parallel for num_threads(numthreads)
   	for(i = 0; i < n; i++)
	  {
        ...
        #pragma omp parallel for private(j,index_X,index_Y) num_threads(numthreads)
	for(i = 0; i<n; i++)
	  {
   		for(j = 0; j < Ones; j++)
		  {
...
void func2(double *weights, double *probability, int n)
{
	...
	#pragma omp parallel for reduction(+:sumWeights) num_threads(numthreads)
   	for(i = 0; i < n; i++)
...
void func3(double *arrayX, double *arrayY, double *weights, double *x_e, double *y_e, int n)
{
    ...
    int i;
        ...
        #pragma omp parallel for reduction(+:estimate_x,estimate_y) num_threads(numthreads)
	for(i = 0; i < n; i++){
